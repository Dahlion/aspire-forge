...
export default function EmsPortal({ tenantId, username, logout }: Props) {
  ...
  useEffect(() => {
    ...
    fetchMyPermissions(tenantId, username)
      .then(setPerms)
      .catch(() => setPerms(DEFAULT_PERMS));
  }, [tenantId, username]);

  const isAdmin = perms.canManageCatalog || perms.canManageRoster || perms.canManageLocations;
      ...
      <div data-bs-theme="dark" style={{ minHeight: '100vh', background: T.bg, paddingBottom: 72, color: T.text }}>
        ...
        <nav style={{
          ...
          <button
            ...
            onClick={() => navigateEms({ kind: 'dashboard' })}
          >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <button
              style={{ background: 'none', border: `1px solid ${T.border}`, borderRadius: 6, color: T.text, padding: '4px 10px', cursor: 'pointer' }}
              onClick={logout}
              title="Sign out"
            >
        ...
        {/* Content */}
        <div className="container-fluid px-3 py-3" style={{ maxWidth: 768 }}>
...
function EmsBottomNav({ route, isAdmin }: { route: EmsRoute; isAdmin: boolean }) {
  ...
  const active = (kind: string) =>
    route.kind === kind ||
    (kind === 'settings' && ['settings', 'agency-config', 'personnel', 'locations', 'catalog', 'reports'].includes(route.kind));

  return (
    <nav style={{
      position: 'fixed', bottom: 0, left: 0, right: 0, height: 64,
      background: T.topBar, zIndex: 300,
      ...
      {tabs.map(tab => (
        <a
          key={tab.kind}
          href={emsPath({ kind: tab.kind } as EmsRoute)}
          style={{
            flex: 1, padding: '6px 0', textDecoration: 'none',
            display: 'flex', flexDirection: 'column', alignItems: 'center',
            color: active(tab.kind) ? T.accent : T.muted,
            transition: 'color 0.15s',
          }}
          onClick={e => { e.preventDefault(); navigateEms({ kind: tab.kind } as EmsRoute); }}
        >
